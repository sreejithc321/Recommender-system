...
def return_recommendations(data,item_name,similarity=similar_PC):
	totals={}
	sum_of_sim={}
	for items in data:
		if items==item_name: continue
		sim=similarity(data,item_name,items)
		if sim<=0: continue
		for item in data[items]:
			# find out items to recomend
			if item not in data[item_name] or data[item_name][item]==0:
				# similarity * Score
				totals.setdefault(item,0)
				totals[item]+=data[items][item]*sim
				# sum of similarities
